...
export const Route = createFileRoute("/services/$slug")({
  loader: async ({ params, context }) => {
    try {
      ...
      return { title: svc.title, description: svc.short_description ?? svc.description ?? undefined, image: svc.featured_image ?? undefined };
    } catch {
      throw notFound();
    }
  },
  head: ({ loaderData, params }) => ({
    meta: [
      ...
      { name: "description", content: loaderData?.description ?? "Home health care service by Nupun." },
          ...
          <div className="lg:col-span-7 space-y-8">
            {service.featured_image && (
              <div className="overflow-hidden rounded-3xl border border-border">
                <img src={service.featured_image} alt={service.title} className="w-full h-auto object-cover" />
              </div>
            )}
            {service.description && (
            ...
            {service.gallery && service.gallery.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {service.gallery.map((g) => (
                  <img key={g} src={g} alt="" className="aspect-square w-full rounded-2xl object-cover" loading="lazy" />
                ))}
              </div>
            )}
          </div>
          <div className="lg:col-span-5">
            <div className="sticky top-24 space-y-5">
              {service.price && (
                <div className="rounded-3xl border border-border bg-gradient-to-br from-primary-soft to-surface p-6">
                  ...
                  <div className="mt-1 font-display text-4xl">
                    ...
                    {service.price_unit && <span className="text-base text-muted-foreground"> / {service.price_unit}</span>}
                  </div>
                </div>
              )}
              <div>
              ...
              <div className="text-center text-sm text-muted-foreground">
                Prefer to talk? <Link to="/contact" className="text-accent font-medium">Contact us →</Link>
              </div>
            </div>
          </div>
        </div>
