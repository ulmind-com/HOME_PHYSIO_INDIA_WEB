...
const EQUIPMENT_ITEMS = [
  { src: "/assets/equipment/ultrasound.jpeg", title: "Siemens X700 Ultrasound", category: "Diagnostic", price: "₹15,000", desc: "Advanced cardiovascular and OB/GYN imaging system with high-res display." },
  { src: "/assets/equipment/monitoring-1.jpeg", title: "At-Home Health Monitoring", category: "Monitoring", price: "₹2,500", desc: "Complete kit for blood pressure, glucose, and pulse oximetry tracking." },
  { src: "/assets/equipment/anesthesiology.jpeg", title: "Anesthesiology Equipment", category: "Life Support", price: "Contact us", desc: "Hospital-grade anesthesia workstation for specialized home care setups." },
  { src: "/assets/equipment/monitoring-2.jpeg", title: "Vitals Testing Kit", category: "Monitoring", price: "₹1,200", desc: "Portable multi-parameter patient monitor for continuous vitals tracking." },
  { src: "/assets/equipment/stethoscope.jpeg", title: "Clinical Stethoscope", category: "Diagnostic", price: "₹500", desc: "Professional acoustic stethoscope for precise cardiac assessment." },
];
    ...
    <>
        ...
        <div className="container-x pt-24 pb-8 lg:pt-24 lg:pb-12 grid lg:grid-cols-12 gap-8 lg:gap-6 items-center">
          ...
          <div className="space-y-4 lg:col-span-7 xl:col-span-6 lg:pr-6">
            ...
            <p className="text-[16px] text-foreground/80 max-w-[480px] leading-relaxed">
              Oxygen concentrators, hospital beds, wheelchairs and more — sanitised, insured and delivered, usually the same day.
            </p>

            {/* Features list (mini) */}
            <div className="flex flex-col gap-2.5 py-1">
              <div className="flex items-center gap-3">
                <div className="h-5 w-5 rounded-full bg-primary/10 flex items-center justify-center text-primary"><Truck className="h-3 w-3" /></div>
                <span className="text-[14px] font-medium text-foreground/90">Same-day delivery across the city</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="h-5 w-5 rounded-full bg-primary/10 flex items-center justify-center text-primary"><ShieldCheck className="h-3 w-3" /></div>
                <span className="text-[14px] font-medium text-foreground/90">Hospital-grade sanitised & insured units</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="h-5 w-5 rounded-full bg-primary/10 flex items-center justify-center text-primary"><Sparkles className="h-3 w-3" /></div>
                <span className="text-[14px] font-medium text-foreground/90">Flexible rental durations with no lock-ins</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              {phone ? (
                <a href={`tel:${phone}`} className="inline-flex items-center gap-2 rounded-full bg-primary border border-primary px-6 py-3 text-[14px] font-semibold text-white shadow-[0_8px_20px_rgba(26,130,118,0.25)] hover:bg-primary/90 hover:-translate-y-0.5 transition-all">
                  <Phone className="h-4 w-4" /> Request equipment
                </a>
              ) : null}
              <Link to="/contact" className="inline-flex items-center gap-2 rounded-full bg-white border border-border/80 px-6 py-3 text-[14px] font-semibold text-foreground shadow-[0_2px_10px_rgba(0,0,0,0.03)] hover:bg-surface hover:-translate-y-0.5 transition-all">
                 Contact support
              </Link>
            </div>
          </div>
          ...
          <div className="grid gap-4 md:gap-5 md:grid-cols-3">
            {PERKS.map((p, i) => (
              <Reveal key={p.title} delay={i * 0.07}>
                <div className={`h-full rounded-[20px] ${p.theme.bg} p-6 lg:p-7 flex items-center gap-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)]`}>
                  {/* Icon Circle */}
                  <div className={`shrink-0 grid h-[60px] w-[60px] place-items-center rounded-full ${p.theme.iconBg} ${p.theme.iconColor}`}>
      ...
      <section className="relative overflow-hidden pt-8 pb-16 lg:pt-10 lg:pb-24 bg-background">
        ...
        <div className="absolute inset-0 z-0 pointer-events-none" aria-hidden="true">
          ...
          <div className="absolute top-[40%] right-[-5%] w-[450px] h-[450px] bg-accent/15 rounded-full mix-blend-multiply filter blur-[90px] opacity-60 animate-float-slower" style={{ animationDelay: '2s' }} />
          <div className="absolute bottom-[5%] left-[20%] w-[600px] h-[600px] bg-blue-300/15 rounded-full mix-blend-multiply filter blur-[120px] opacity-50 animate-float-slow" style={{ animationDelay: '4s' }} />
        </div>
        ...
        <div className="container-x relative z-10 max-w-7xl mx-auto">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {EQUIPMENT_ITEMS.map((item, i) => (
              <div key={i} className="group relative flex h-full flex-col overflow-hidden rounded-[2.5rem] border border-white/60 bg-white/40 p-3 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.04),inset_0_0_0_1px_rgba(255,255,255,0.4)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(26,130,118,0.15)] hover:border-primary/40 hover:bg-white/60 cursor-pointer">
                ...
                <div className="p-6 flex flex-col flex-1 relative z-10">
                  ...
                  <div className="mt-auto pt-7 flex items-end justify-between">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-1">Rental Rate</span>
                      <div className="flex items-baseline gap-1.5">
                        <span className="font-display text-[26px] font-bold text-foreground">{item.price}</span>
                        {item.price !== "Contact us" && <span className="text-[13px] font-semibold text-muted-foreground">/ day</span>}
                      </div>
                    </div>
