...
function AboutPage() {
  ...
  const VALUES = settings?.about_values?.length
    ? settings.about_values.map((v, i) => ({ icon: ICONS[i % ICONS.length], title: v.title, body: v.body }))
    : DEFAULT_VALUES;
  ...
  const heroDesc = settings?.about_hero_description || "World-class medical care with compassion and innovation, for you and your family.";
    ...
    <>
        ...
        <div className="container-x pt-24 pb-6 md:pb-8 lg:pt-28 lg:pb-10 grid lg:grid-cols-12 gap-6 lg:gap-6 items-center">
          ...
          <div className="space-y-4 lg:col-span-7 xl:col-span-6 lg:pr-6">
            ...
            <div className="flex flex-wrap gap-3 md:gap-5 pt-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mt-0.5">Years of Trust</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-primary"><Stethoscope className="h-9 w-9" strokeWidth={1.5} /></div>
                <div>
                  <div className="font-display text-[22px] font-bold leading-tight">4000+</div>
                  <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mt-0.5">Expert Doctors</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-primary"><HeartPulse className="h-9 w-9" strokeWidth={1.5} /></div>
                <div>
                  <div className="font-display text-[22px] font-bold leading-tight">30M+</div>
                  <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mt-0.5">Lives Touched</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3 pt-0">
              ...
              <Link to="/services" className="inline-flex items-center gap-2 rounded-full bg-white border border-border/80 px-5 py-2.5 text-[13px] font-semibold text-foreground shadow-[0_2px_10px_rgba(0,0,0,0.03)] hover:bg-surface hover:-translate-y-0.5 transition-all">
      ...
      <Section className="relative pt-16 lg:pt-24 pb-16 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden" aria-hidden="true">
          ...
          <div className="absolute bottom-[10%] right-[10%] w-[600px] h-[600px] bg-[#2E9296]/20 rounded-full mix-blend-multiply filter blur-[120px] opacity-60 animate-float-slower" style={{ animationDelay: '2s' }} />
          <div className="absolute top-[40%] left-[60%] w-[400px] h-[400px] bg-[#7DF8D6]/30 rounded-full mix-blend-multiply filter blur-[90px] opacity-60 animate-float-slow" style={{ animationDelay: '1s' }} />
        </div>
          ...
          <div className="relative overflow-hidden rounded-[2.5rem] bg-white/40 backdrop-blur-3xl border border-white/80 shadow-[0_8px_32px_rgba(0,0,0,0.04),inset_0_0_0_1px_rgba(255,255,255,0.5)] lg:col-span-5 p-10 lg:p-14 flex flex-col justify-center transition-all hover:bg-white/50">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <h2 className="font-display text-3xl tracking-tight text-foreground md:text-4xl lg:text-5xl leading-[1.1]">
                {settings?.about_story_title || "The best care doesn't happen in corridors — it happens at home."}
              </h2>
              <div className="space-y-4 text-[15px] leading-relaxed text-foreground/80">
                {settings?.about_story_text ? (
                  ...
                  <>
                    <p>
                      {name} began with a simple observation: the best care is delivered by people who show up on
                    ...
                    <p>
                      ...
                      assessed and continuously trained.
                    </p>
              ...
              {VALUES.map((v, i) => (
                <Reveal key={v.title} delay={i * 0.05}>
                  <div className="group h-full flex flex-col rounded-3xl border border-white/60 bg-white/50 backdrop-blur-lg p-6 shadow-sm transition-all duration-500 hover:-translate-y-1 hover:shadow-xl hover:shadow-primary/10 hover:border-white hover:bg-white/80">
                    ...
                    <div className="font-display text-xl font-bold text-foreground transition-colors group-hover:text-primary">{v.title}</div>
                    <div className="mt-2 text-sm leading-relaxed text-muted-foreground">{v.body}</div>
                  </div>
                </Reveal>
              ))}
            </div>
      ...
      <Section>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {PROCESS.map((p, i) => {
            const themes = [
              { bg: "bg-[#F5FAF6]", iconBg: "bg-[#E5F4EA]", iconColor: "text-[#16A34A]", border: "border-[#E5F4EA]" },
              { bg: "bg-[#F4F7FC]", iconBg: "bg-[#E6F0FD]", iconColor: "text-[#2563EB]", border: "border-[#E6F0FD]" },
              { bg: "bg-[#FDF5FB]", iconBg: "bg-[#FCE6F5]", iconColor: "text-[#C026D3]", border: "border-[#FCE6F5]" },
            ];
            const theme = themes[i % themes.length];
            
            return (
              <Reveal key={p.step} delay={i * 0.08}>
                <div className={`group relative h-full rounded-[2rem] border ${theme.border} ${theme.bg} p-6 transition-all duration-500 hover:shadow-xl hover:-translate-y-1 flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-5`}>
                  {/* Glowing Icon Wrapper */}
                  <div className={`shrink-0 relative grid h-20 w-20 place-items-center rounded-full ${theme.iconBg} ${theme.iconColor} transition-transform duration-500 group-hover:scale-105`}>
                    {/* Inner glowing pulse */}
