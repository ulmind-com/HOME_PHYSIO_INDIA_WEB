  ...
  head: () => ({
    meta: [
      { title: "Careers — Join Nupun Home Health Care" },
      {
        name: "description",
        content:
          "Join a caregiving team where clinical excellence meets compassion. Open roles for nurses, physios and more.",
      },
      { property: "og:title", content: "Careers — Nupun Home Health Care" },
      {
        property: "og:description",
        content: "Open roles for nurses, physios and care coordinators.",
      },
...
const PERKS = [
  {
    icon: TrendingUp,
    title: "Real growth",
    body: "Structured training, certifications and a clear path from attendant to senior nurse.",
  },
  {
    icon: ShieldCheck,
    title: "Respect & safety",
    body: "Fair pay, on-time payouts, insurance and a team that has your back on every shift.",
  },
  {
    icon: GraduationCap,
    title: "Learn continuously",
    body: "Access our NCR training hub and clinical mentorship from senior practitioners.",
  },
  {
    icon: HeartHandshake,
    title: "Meaningful work",
    body: "Care for people who need you most — and go home knowing the day mattered.",
  },
];
        ...
        {isLoading ? (
          <div className="grid gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="h-28 rounded-3xl border border-border bg-surface animate-pulse"
              />
            ))}
          </div>
        ) : items.length ? (
          ...
          <EmptyState
            ...
            description="Send us your resume — we'll reach out when a fit opens up."
          />
        )}
      </Section>
      ...
      <Section className="pt-0">
        <div className="rounded-[2.5rem] border border-border bg-gradient-to-br from-primary-soft to-surface p-10 text-center lg:p-16">
          ...
          <p className="mx-auto mt-4 max-w-xl text-muted-foreground">
            We're always looking for exceptional caregivers. Tell us about yourself and we'll be in
            touch when the right opening appears.
          </p>
          <Link
            to="/contact"
