      ...
      <div className="relative aspect-[4/3] overflow-hidden rounded-[2rem] bg-primary/5 shadow-inner">
        {equipment.featured_image ? (
          ...
          <div className="grid h-full w-full place-items-center">
            <svg viewBox="0 0 24 24" className="h-10 w-10 text-primary/40" fill="none" stroke="currentColor" strokeWidth="1.5">
          ...
          {price ? (
            <div className="flex flex-col">
              ...
              <div className="flex items-baseline gap-1.5">
                <span className="font-display text-[26px] font-bold text-foreground">₹{price.toLocaleString()}</span>
                <span className="text-[13px] font-semibold text-muted-foreground">/ {equipment.price_unit ?? "day"}</span>
              </div>
            </div>
          ) : (
