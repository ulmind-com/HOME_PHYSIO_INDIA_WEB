...
export function TestimonialCard({ t }: { t: Testimonial & Record<string, any> }) {
  ...
  return (
    <figure className="group flex h-full flex-col justify-between rounded-3xl border border-border/80 bg-background dark:bg-card/40 p-7 sm:p-8 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-primary/40 relative overflow-hidden">
      ...
      <div>
        ...
        <blockquote className="text-base sm:text-[17px] leading-relaxed text-foreground/90 font-normal">
          “{text}”
        </blockquote>
      </div>

      {/* Author Footer (Photo + Name + Designation) */}
      <figcaption className="mt-8 flex items-center gap-4 pt-5 border-t border-border/40">
        {avatar ? (
          <div className="relative shrink-0">
            <img
              src={avatar}
              alt={name}
              className="h-12 w-12 rounded-full object-cover shadow-sm ring-2 ring-primary/20 group-hover:ring-primary/50 transition-all duration-300"
            />
          </div>
        ) : (
          <div className="shrink-0 h-12 w-12 rounded-full bg-primary/10 border border-primary/20 grid place-items-center text-primary font-display text-lg font-bold group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-300 shadow-sm">
            {name?.[0]?.toUpperCase() ?? "U"}
          </div>
        )}

        <div className="min-w-0 flex-1">
          <div className="font-display text-[16px] font-semibold text-foreground group-hover:text-primary transition-colors duration-300 truncate">
            {name}
          </div>
          {role && (
