...
const STEPS = [
  ...
  { key: "patient", label: "Patient", fields: ["patient_name", "patient_age", "patient_gender", "message"] as const },
  { key: "contact", label: "Contact", fields: ["contact_phone", "contact_email", "address", "city", "pincode"] as const },
];
...
export function BookingForm({ presetServiceSlug }: { presetServiceSlug?: string } = {}) {
  ...
  if (done) return <SuccessState onReset={() => { setDone(false); setStep(0); form.reset(); }} />;
    ...
    <div className="relative overflow-hidden rounded-[2rem] border border-border bg-gradient-to-b from-surface to-background/60 shadow-[var(--shadow-float)]">
      ...
      <div className="relative border-b border-border/70 px-6 pt-6 pb-4 lg:px-9 lg:pt-8">
        <div className="flex items-center justify-between text-xs uppercase tracking-[0.18em]">
          <span className="text-accent font-semibold">Step {step + 1} of {STEPS.length}</span>
      ...
      >
        ...
        <div className="mt-10 flex items-center justify-between gap-3">
          ...
          >
            ...
            {!mut.isPending && <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />}
          </button>
        </div>
      </form>
    </div>
...
function ServiceStep({ form, options }: { form: UseFormReturn<Values>; options: { id: string; title: string; slug: string; short_description?: string | null }[] }) {
  ...
  return (
    <div className="grid gap-6">
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {s.short_description && (
                    <div className="mt-1 text-xs text-muted-foreground line-clamp-2">{s.short_description}</div>
                  )}
                </div>
                <div className={cn("grid h-5 w-5 shrink-0 place-items-center rounded-full border", active ? "border-primary bg-primary" : "border-border")}>
      ...
      <div className="mt-2">
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground mb-3">Care type</div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? "border-primary bg-primary text-primary-foreground" : "border-border bg-surface hover:border-primary/50",
                )}
              >
                {o.label}
              </button>
...
function ScheduleStep({ form }: { form: UseFormReturn<Values> }) {
  return (
    <div className="grid gap-6">
      <StepHeader eyebrow="When" title="When should we arrive?" description="Pick your preferred date and time — final slot is confirmed on the call." />
      ...
      <div className="rounded-2xl border border-border bg-primary-soft/40 p-4 text-sm text-muted-foreground flex items-start gap-3">
        ...
        <span>Our care desk is open 24/7. A dedicated advisor will confirm your slot within 2 hours.</span>
      </div>
    </div>
  );
}
...
function ContactStep({ form }: { form: UseFormReturn<Values> }) {
  return (
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <FloatField label="City"><input {...form.register("city")} className={inputCls} /></FloatField>
        <FloatField label="Pincode"><input {...form.register("pincode")} className={inputCls} /></FloatField>
      </div>
    </div>
  );
}
...
function StepHeader({ eyebrow, title, description }: { eyebrow: string; title: string; description?: string }) {
  return (
    <div>
      <div className="text-[11px] uppercase tracking-[0.22em] text-accent font-semibold">{eyebrow}</div>
...
function FloatField({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
  ...
  return (
    ...
    >
      ...
      <p className="relative mt-3 text-muted-foreground max-w-md mx-auto">
        A care advisor will call you within <b className="text-foreground">2 hours</b> to confirm your slot and match a verified caregiver.
      </p>
      <button
        onClick={onReset}
